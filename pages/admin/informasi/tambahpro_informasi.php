    ...
    if ($upload_error === UPLOAD_ERR_OK) {
        if (move_uploaded_file($loc_image, "../img/artikel/$img")) {
            try {
                ...
                $pdo = $conn->prepare('INSERT INTO tbl_informasi (kd_penyakit, nama_penyakit, judul, deskripsi, pencegahan, img)
                                    values (:kd_penyakit, :nama_penyakit, :judul, :deskripsi, :pencegahan, :img)');
                ...
                if ($pdo->execute($insertdata)) {
                    // Reset IDs to be sequential without gaps
                    $conn->exec("SET @new_id = 0");
                    $conn->exec("UPDATE tbl_informasi SET id = (@new_id := @new_id + 1) ORDER BY id");

                    // Reset auto_increment to max(id) + 1
                    $maxIdStmt = $conn->query("SELECT MAX(id) AS max_id FROM tbl_informasi");
                    $maxId = $maxIdStmt->fetch(PDO::FETCH_ASSOC)['max_id'];
                    $nextId = $maxId + 1;
                    $conn->exec("ALTER TABLE tbl_informasi AUTO_INCREMENT = $nextId");
                    ...
                    $content = '
                        ...
                        <h1 class="text-2xl font-bold text-gray-800 mb-4">Informasi Berhasil Ditambahkan!</h1>
                        <p class="text-gray-600 mb-6">Data informasi baru telah berhasil disimpan ke dalam sistem.</p>
                    ...
                    $content = '
                        ...
                        <p class="text-gray-600 mb-6">Terjadi kesalahan saat menyimpan data informasi. Silakan coba lagi.</p>
                        <a href="?pages=tambah_informasi" class="inline-block px-6 py-3 rounded-xl font-semibold text-white btn-error">
                ...
                $content = '
                    ...
                    <h1 class="text-2xl font-bold text-gray-800 mb-4">Tambah Informasi Gagal</h1>
            ...
            $content = '
                ...
                <h1 class="text-2xl font-bold text-gray-800 mb-4">Gagal Memindahkan File Gambar</h1>
                <p class="text-gray-600 mb-6">Terjadi kesalahan saat memindahkan file gambar ke direktori tujuan.</p>
                <a href="?pages=tambah_informasi" class="inline-block px-6 py-3 rounded-xl font-semibold text-white btn-error">
                    <i class="fas fa-arrow-left mr-2"></i> Kembali ke Form
                </a>
            ';
        ...
        $content = '
            ...
            <h1 class="text-2xl font-bold text-gray-800 mb-4">Gagal Upload Gambar</h1>
            <p class="text-gray-600 mb-6">Terjadi kesalahan saat mengupload file gambar. Error code: ' . $upload_error . '</p>
            <a href="?pages=tambah_informasi" class="inline-block px-6 py-3 rounded-xl font-semibold text-white btn-error">
                <i class="fas fa-arrow-left mr-2"></i> Kembali ke Form
            </a>
        ';
