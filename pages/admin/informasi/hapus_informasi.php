...
if (!isset($_GET['id'])) {
    renderErrorPage("ID informasi tidak ditemukan.");
    exit;
}
...
try {
    ...
    $stmt = $conn->prepare('DELETE FROM tbl_informasi WHERE id = :id');
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);

    if ($stmt->execute()) {
        renderSuccessPage();
    } else {
        renderErrorPage("Gagal menghapus data informasi.");
    }
} catch (PDOException $e) {
    renderErrorPage("Error saat menghapus informasi: " . $e->getMessage());
    exit;
}
...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
    <div class="glass-card p-8 max-w-md w-full text-center success-animation">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-3">Informasi Berhasil Dihapus!</h2>
        <p class="text-gray-600 mb-6">Data informasi penyakit telah berhasil dihapus dari sistem.</p>
        <div class="flex justify-center">
            <a href="?pages=data_informasi" class="btn-primary px-6 py-3 rounded-lg text-white font-medium inline-flex items-center">
                <i class="fas fa-arrow-left mr-2"></i> Kembali ke Data Informasi
            </a>
        </div>
    </div>
    <script>
...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
    <div class="glass-card p-8 max-w-md w-full text-center">
        ...
        <div class="flex justify-center space-x-4">
            <a href="?pages=data_informasi" class="btn-error px-6 py-3 rounded-lg text-white font-medium inline-flex items-center">
