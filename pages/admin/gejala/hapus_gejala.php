...
if (!isset($_GET['kd_gejala'])) {
    renderErrorPage("Kode gejala tidak ditemukan.");
    exit;
}
...
try {
    ...
    $stmt = $conn->prepare('DELETE FROM tbl_gejala WHERE kd_gejala = :kd_gejala');
    $stmt->bindParam(':kd_gejala', $kd_gejala, PDO::PARAM_STR);

    if ($stmt->execute()) {
        renderSuccessPage();
    } else {
        renderErrorPage("Gagal menghapus data gejala.");
    }
} catch (PDOException $e) {
    renderErrorPage("Error saat menghapus gejala: " . $e->getMessage());
    exit;
}
...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
    <div class="glass-card p-8 max-w-md w-full text-center success-animation">
        ...
        <h2 class="text-2xl font-bold text-gray-800 mb-3">Gejala Berhasil Dihapus!</h2>
        <p class="text-gray-600 mb-6">Data gejala telah berhasil dihapus dari sistem.</p>
        <div class="flex justify-center">
            <a href="?pages=data_gejala" class="btn-primary px-6 py-3 rounded-lg text-white font-medium inline-flex items-center">
                <i class="fas fa-arrow-left mr-2"></i> Kembali ke Data Gejala
            </a>
        </div>
    </div>
    <script>
...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 p-4">
    <div class="glass-card p-8 max-w-md w-full text-center">
        ...
        <div class="flex justify-center space-x-4">
            <a href="?pages=data_gejala" class="btn-error px-6 py-3 rounded-lg text-white font-medium inline-flex items-center">
