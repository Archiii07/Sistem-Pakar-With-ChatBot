...
if (isset($_POST['submit']) || isset($_POST['confirmed'])) {
    $old_kd_gejala = $_POST['old_kd_gejala'];
    $kd_gejala = $_POST['kd_gejala'];
    $gejala = htmlspecialchars($_POST['gejala']);

    try {
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $pdo = $conn->prepare('UPDATE tbl_gejala SET kd_gejala = :kd_gejala, gejala = :gejala WHERE kd_gejala = :old_kd_gejala');
        $params = array(
            ':kd_gejala' => $kd_gejala,
            ':gejala' => $gejala,
            ':old_kd_gejala' => $old_kd_gejala
        );
        ...
        if ($pdo->execute($params)) {
            $content = '
                ...
                <h1 class="text-2xl font-bold text-gray-800 mb-4">Gejala Berhasil Diperbarui!</h1>
                <p class="text-gray-600 mb-6">Perubahan data gejala telah berhasil disimpan ke dalam sistem.</p>
            ...
            $content = '
                ...
                <p class="text-gray-600 mb-6">Terjadi kesalahan saat menyimpan perubahan data gejala.</p>
    ...
    $content = '
        ...
        <a href="?pages=data_gejala" class="inline-block px-6 py-3 rounded-xl font-semibold text-white btn-error">
            <i class="fas fa-clipboard-list mr-2"></i> Kembali ke Data Gejala
        </a>
    ';
    renderPage($content);
